...
const Header: React.FC = () => {
  ...
  return (
    <header className="flex items-center justify-between p-4 bg-white shadow-md fixed top-0 w-full z-10">
      ...
      <div>
        <img src={logo} alt="Kyoto" className="rounded-full size-[4rem]" />
      </div>
          ...
          <SheetTrigger>
            <div className="flex items-center">
              <ShoppingCart
                ...
                size={32}
              />
              <span className="relative right-[0.2rem] bottom-2.5 flex items-center justify-center text-lg text-black bg-[#f06139] border-2 border-black rounded-full size-6 font-semibold">
                {countItemsInCart}
              </span>
            </div>
          </SheetTrigger>
